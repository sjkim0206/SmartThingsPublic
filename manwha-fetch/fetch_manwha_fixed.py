...
def send_to_kakao(files:list):
    """
    PNG 파일 목록을 Tasker로 전달하여 카카오톡 단톡방에 전송.

    동작 흐름:
      1. kakao_send.json 에 파일 경로와 대상 방 이름을 저장
      2. Tasker 태스크(KAKAO_TASK_NAME)를 am broadcast로 트리거
      3. Tasker가 AutoInput으로 각 방에 이미지를 공유

    Tasker 태스크 구성 예시 (태스크 이름: "카톡만평전송"):
      [1] Read File  → Variable:%KFILES  ← /sdcard/Pictures/naver_manhwa/kakao_send.json
      [2] JSON Parse → %KFILES 에서 files 배열, rooms 배열 추출
      [3] For loop   → rooms 각 방에 대해:
            [3-1] Send Intent
                  Action   : android.intent.action.SEND_MULTIPLE
                  MimeType : image/*
                  Extra    : android.intent.extra.STREAM = file://[각 파일 경로]
                  Package  : com.kakao.talk
            [3-2] Wait 2초
            [3-3] AutoInput Text  → 방 이름 검색 & 탭
            [3-4] AutoInput Action → 전송 버튼 탭
            [3-5] Wait 3초
    """
    if not files:
...
def _fallback_share(files:list):
    """
    Tasker 없을 때 termux-share로 파일 하나씩 공유 (수동 선택 필요).
    """
